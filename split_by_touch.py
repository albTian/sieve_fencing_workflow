...
def SplitVideoByTouch(video: sieve.Video) -> str:
    ...
    def record_touch(cap, position, length, clip_num, left_score, right_score):
        ...
        output_file = f"videos-cut/test-{clip_num}[{left_score}-{right_score}].mp4"
    ...
    while position < cap_end_point:
        ...
        red_light = (np.sum(abs(frame[330:334, 140:260].astype(int)-green_box_int)) <= 40000)
        green_light = (np.sum(abs(frame[330:334, 380:500].astype(int)-red_box_int)) <= 40000)
        if red_light or green_light:
            ...
            # print(f"{left_score}, {right_score}, {position}")
            record_touch(cap, position-60, 90, clips_recorded, left_score, right_score)
            clips_recorded += 1
